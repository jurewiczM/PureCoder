...
```python
"""The contract layer: schema guards, anchors, rendering, derivation."""
...
**Interfaces:**
- Consumes: contract dicts validated by `validate_contract` from Task 1.
...
Add `import ast` and extend the `purecoder.contract` import at the top of the
file to `from purecoder.contract import anchor_tests, validate_contract`.

- [ ] **Step 2: Run tests to verify they fail**

Run: `.venv/bin/python -m pytest tests/test_contract.py -q`
Expected: FAIL — `ImportError: cannot import name 'anchor_tests'`

- [ ] **Step 3: Implement anchor generation**

Append to `purecoder/contract.py`, and add `import ast`, `import builtins`,
`import re` to the imports at the top:
...
git commit -m "feat(contract): generate anchor assertions from contract examples"
```
...
```python
from .contract import anchor_tests, derive_contract, render_contract
```
...
| 5 | `anchor_tests` (mechanical assertions) | ✅ tested | both shapes, malformed and custom-exception drops |
...
```
  contract.py    prose → grammar-constrained contract → mechanical anchor tests
```
